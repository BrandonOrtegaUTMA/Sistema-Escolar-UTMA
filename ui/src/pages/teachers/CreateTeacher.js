...
function CreateTeacher() {
    ...
    const submit = (e) => {
        ...
        e.preventDefault();
        Swal.fire("Maestro", "Guardando maestro...");
        Swal.showLoading();
        Service.post("/teacher", formData, false)
        .then(response =>{
            if(response.data){
                window.location.href = '/teachers/list'
            }else if (response.status === 402) {
                Swal.fire("Error al crear maestro", "La dirección de correo ya existe", "error");
            }else{
                Swal.fire("Nuevo maestro", "Ocurrió un error al guardar al maestro", "error");
            }
        })
    }
    ...
    <div id="teacher-form">
        <Row>
            <Col xs={12} md={{span:8,offset:2}}>
                <Card>
                    <Card.Body>
                        <Card.Title>Agregar maestro</Card.Title>
                        <Form onSubmit={submit}  autoComplete="off">
                            ...
                            <Row className="mt-5">
                                ...
                                <Col xs={12} md={2}>
                                    <a className="ml-3 btn btn-danger w-100" href="/teacher/list">
                                        Cancelar
                                    </a>
                                </Col>
                            </Row>
